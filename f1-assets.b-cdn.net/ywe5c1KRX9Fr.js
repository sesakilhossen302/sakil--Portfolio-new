...
(function() {
  ...
  const Sidebar = {
    ...
    scale() {
      // Disable auto-scale on mobile/tablet devices (< 992px)
...
const MobileMenu = {
  ...
  init() {
    ...
    if (this.menuIcons.length > 0) {
      const iconHeight = this.menuIcons[0].offsetHeight || 16;
    ...
    console.log('✓ Mobile Menu initialized for <992px');
  },
  ...
  const GhostEngine = {
    ...
    destroy() {
      ...
      const ghostElements = [
        '.nav-logo-item .sakil-logo',
        '.sakil-copyright-icon',
        '.nav-button',
    ...
    measurePair(real, ghost, type) {
      ...
      if (type === 'logo') {
        const parent = real.closest('.sakil-copyright-wrap') || real.closest('.nav-button-wrap') || real.closest('.nav-logo-item') || real.parentElement;
    ...
    applyAnimation(m) {
      ...
      if (type === 'logo') {
        ...
        if (!parent.classList.contains('sakil-copyright-wrap') && !parent.classList.contains('nav-button-wrap')) {
    ...
    createAll() {
      ...
      measurements.push(this.measurePair(Utils.$('.nav-logo-item .sakil-logo'), Utils.$('.sakil-logo-ghost'), 'logo'));
      ...
      // .nav-logo-item to position:relative, which shifts .sakil-copyright-wrap
      ...
      const crReal = Utils.$('.sakil-copyright-icon');
      const crGhost = Utils.$('.sakil-copyright-icon-ghost');
      if (crReal && crGhost) {
        const parent = crReal.closest('.sakil-copyright-wrap');
...
const HorizontalScroll = {
  ...
  init() {
    // Disable HorizontalScroll on mobile devices and tablets (< 992px)
    if (window.innerWidth < 992) {
      ...
      if (stickySupportEl) {
        stickySupportEl.style.display = 'none';
        stickySupportEl.style.height = '0px';
      }
      if (stickyEl) {
  ...
  performSetup(section, stickyEl, stickySupportEl, wrap, track) {
    ...
    if (stickySupportEl) stickySupportEl.offsetHeight;
    wrap.offsetHeight;
    track.offsetHeight;
    ...
    stickyEl.style.maxHeight = '100vh';
    stickyEl.style.zIndex = '20';

    // 3. Compute accurate horizontal scroll bounds for all 12 cards
    const wrapRect = wrap.getBoundingClientRect();
    const trackRect = track.getBoundingClientRect();
    // Extra breathing room at the end so Card 12 is fully visible
    const overflowRight = Math.max(0, trackRect.right - wrapRect.right + (window.innerWidth * 0.08));
    const xMovement = -overflowRight;
    
    // 4. Calculate smooth scroll distance based on all 12 cards
    const viewportHeight = window.innerHeight;
    const scrollDistance = Math.max(viewportHeight * 3.8, Math.round(overflowRight * 0.95));
    const sectionHeight = scrollDistance + viewportHeight;
    
    section.style.minHeight = sectionHeight + 'px';
    section.style.height = sectionHeight + 'px';

    // 5. Calculate and set work-sticky-support height
    const stickyHeight = stickyEl.offsetHeight || viewportHeight;
    const supportHeight = Math.max(0, sectionHeight - stickyHeight);
    if (stickySupportEl) {
      stickySupportEl.style.display = 'block';
    ...
    STATE.workTween = gsap.to(track, {
      ...
      scrollTrigger: {
        ...
        onRefresh: () => {
          ...
          const newOverflowRight = Math.max(0, newTrackRect.right - newWrapRect.right + (window.innerWidth * 0.08));
          
          const newViewportHeight = window.innerHeight;
          const newScrollDistance = Math.max(newViewportHeight * 3.8, Math.round(newOverflowRight * 0.95));
          const newSectionHeight = newScrollDistance + newViewportHeight;
          section.style.minHeight = newSectionHeight + 'px';
          section.style.height = newSectionHeight + 'px';

          const newStickyHeight = stickyEl.offsetHeight || newViewportHeight;
          const newSupportHeight = Math.max(0, newSectionHeight - newStickyHeight);
          if (stickySupportEl) {
    ...
    // Ensure all 12 cards are 100% visible and interactive
    const workCards = Utils.$$('.work-card');
    if (workCards.length) {
      gsap.set(workCards, { y: '0%', opacity: 1, scale: 1 });
    }
...
const Preloader = {
  ...
  init() {
    ...
    const logo = Utils.$('.sakil-logo-preload-svg');
    const wrapper = Utils.$('.sakil-logo-wrap');
    if (!logo || !wrapper) return;

    const letters = Utils.$$('.sakil-logo-letter');
    ...
    // Captures .nav-logo-item which contains both the real nav logo (.sakil-logo)
...
const MagneticPositions = {
  ...
  collectPairs() {
    document.querySelectorAll('[data-origin]').forEach(target => {
      ...
      if (target.classList.contains('capa-card') && window.innerWidth < 992) return;
...
const CTAAnimation = {
  ...
  const ImageTrail = {
        images: [
      'sakil_assets/trail/sakil_1.jpg',
      'sakil_assets/trail/sakil_2.jpg',
      'sakil_assets/trail/flower_1.jpg',
      'sakil_assets/trail/sakil_3.jpg',
      'sakil_assets/trail/flower_2.jpg',
      'sakil_assets/trail/sakil_4.jpg',
      'sakil_assets/trail/flower_5.jpg',
      'sakil_assets/trail/sakil_5.jpg',
      'sakil_assets/trail/flower_3.jpg',
      'sakil_assets/trail/sakil_6.jpg',
      'sakil_assets/trail/flower_4.jpg',
      'sakil_assets/trail/sakil_7.jpg',
      'sakil_assets/trail/flower_6.jpg',
      'sakil_assets/trail/sakil_1.jpg',
      'sakil_assets/trail/sakil_2.jpg'
    ],
    config: {
